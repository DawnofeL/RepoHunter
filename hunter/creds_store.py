...
def load_creds() -> dict:
    ...
    blank = {"deepseek_api_key": "", "github_pat": "", "model": ""}
...
def save_creds(deepseek_api_key: str, github_pat: str, model: str = "") -> None:
    """把这次填的凭证和模型写进本地文件覆盖旧的，下次自动带出来。全空就不写。"""
    if not deepseek_api_key and not github_pat and not model:
        return
    CREDS_PATH.parent.mkdir(parents=True, exist_ok=True)
    CREDS_PATH.write_text(
        json.dumps({"deepseek_api_key": deepseek_api_key, "github_pat": github_pat, "model": model},
